    ...
    <div class="page-container max-w-2xl">
        ...
        <div class="page-header animate-fade-in">
            <div class="flex items-center gap-4">
                ...
                <div>
                    ...
                    <p class="text-slate-500 dark:text-slate-400 mt-1 text-sm">{{ $contact->name }}</p>
                </div>
            </div>
        </div>
        ...
        <div class="card animate-fade-in-up" style="opacity: 0; animation-delay: 0.1s;">
            <form method="POST" action="{{ route('contacts.update', $contact) }}" class="p-6 space-y-6">
                ...
                <div>
                    <label for="type" class="form-label">Tipe <span class="text-rose-500">*</span></label>
                    <select id="type" name="type" required class="form-input @error('type') border-rose-500 @enderror">
                        <option value="supplier" {{ old('type', $contact->type) === 'supplier' ? 'selected' : '' }}>Supplier</option>
                        <option value="customer" {{ old('type', $contact->type) === 'customer' ? 'selected' : '' }}>Pelanggan</option>
                    </select>
                    @error('type')
                        <p class="mt-1 text-sm text-rose-500">{{ $message }}</p>
                    @enderror
                ...
                <div>
                    ...
                    <textarea id="address" name="address" rows="3"
                              class="form-input @error('address') border-rose-500 @enderror">{{ old('address', $contact->address) }}</textarea>
                    @error('address')
                        <p class="mt-1 text-sm text-rose-500">{{ $message }}</p>
                    @enderror
                </div>
                ...
                <div>
                    ...
                    <textarea id="notes" name="notes" rows="2"
                              class="form-input @error('notes') border-rose-500 @enderror">{{ old('notes', $contact->notes) }}</textarea>
                    @error('notes')
                        <p class="mt-1 text-sm text-rose-500">{{ $message }}</p>
                    @enderror
                </div>
                ...
                <div>
                    <label class="flex items-center gap-3">
                        ...
                        <span class="text-sm text-slate-700 dark:text-slate-300">Kontak Aktif</span>
                    </label>
                </div>
