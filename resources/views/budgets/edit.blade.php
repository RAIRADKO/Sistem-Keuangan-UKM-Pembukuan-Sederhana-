    ...
    <div class="page-container max-w-xl">
        ...
        <div class="page-header animate-fade-in">
            <div class="flex items-center gap-4">
                ...
                <div>
                    <h1 class="page-title">Edit Anggaran</h1>
                    <p class="text-slate-500 dark:text-slate-400 mt-1 text-sm">{{ $budget->name }}</p>
                </div>
            </div>
        </div>
        ...
        <div class="card animate-fade-in-up" style="opacity: 0; animation-delay: 0.1s;">
            <form method="POST" action="{{ route('budgets.update', $budget) }}" class="p-6 space-y-6">
                @csrf
                @method('PUT')
                ...
                <div class="grid grid-cols-2 gap-4">
                    <div>
                        ...
                        <div class="relative">
                            ...
                            <input type="number" id="amount" name="amount" value="{{ old('amount', $budget->amount) }}" required min="0"
                ...
                <div>
                    ...
                    <div class="relative">
                        <input type="number" id="alert_threshold" name="alert_threshold" value="{{ old('alert_threshold', $budget->alert_threshold) }}" required min="1" max="100"
                ...
                <div>
                    ...
                    <textarea id="notes" name="notes" rows="2"
                              class="form-input @error('notes') border-rose-500 @enderror">{{ old('notes', $budget->notes) }}</textarea>
                    @error('notes')
                        <p class="mt-1 text-sm text-rose-500">{{ $message }}</p>
                    @enderror
                </div>
                ...
                <div>
                    <label class="flex items-center gap-3">
                        ...
                        <span class="text-sm text-slate-700 dark:text-slate-300">Anggaran Aktif</span>
                    </label>
                </div>
